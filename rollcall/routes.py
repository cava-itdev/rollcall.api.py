...
def register():
    ...
    if not photoId: return jsonify(message='photoId not provided'), http.HTTPStatus.BAD_REQUEST
    member = req_data.get('member') # member is a dictionary
    if not member: return jsonify(message='member not provided'), http.HTTPStatus.BAD_REQUEST
    ...
    try:
        ...
        if photoId: status = http.HTTPStatus.CREATED if member else http.HTTPStatus.NOT_FOUND
        else: status = http.HTTPStatus.BAD_REQUEST
    except Exception:
